      ...
      <div className="max-w-[720px] mx-auto px-6 sm:px-10 lg:px-0 pt-16 sm:pt-24 pb-28">
        ...
        <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <div className="flex flex-wrap gap-3 mb-14">
            ...
            <Link to="/mock">
              <motion.button whileTap={{ scale: 0.97 }}
                className="h-10 px-5 bg-[var(--color-bg2)] border border-[var(--color-border)] text-[var(--color-text2)] text-[13px] font-medium rounded-lg hover:text-white hover:border-[var(--color-border2)] transition-all cursor-pointer">
                Mock Interview
              </motion.button>
            </Link>
          </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
          ...
          <div>
            <div className="text-[22px] font-bold text-white">{questions.length}</div>
            <div className="text-[12px] text-[var(--color-text3)]">Questions</div>
          </div>
          <div>
            <div className="text-[22px] font-bold text-white">{sections.length}</div>
                  ...
                  <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-0.5">
                        <h3 className="text-[14px] font-semibold text-white group-hover:text-[var(--color-accent)] transition-colors leading-snug">
                          {section.name}
                        </h3>
                        <span className="text-[var(--color-text3)] group-hover:text-[var(--color-accent)] transition-colors text-[12px]">→</span>
                      </div>
                      <p className="text-[12px] text-[var(--color-text3)] leading-relaxed mb-1.5">{section.description}</p>
